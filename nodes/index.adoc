...
* xref:../nodes/nodes/nodes-nodes-working.html#nodes-nodes-working-deleting_nodes-nodes-working[Delete a node from a cluster] by scaling down the cluster using a compute machine set. To delete a node from a bare-metal cluster, you must first drain all pods on the node and then manually delete the node.
