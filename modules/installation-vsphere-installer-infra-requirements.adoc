...
* Using Storage vMotion is not supported and can cause issues.

* Using vMotion to migrate machine workloads is generally supported, where _generally_ implies that you meet all VMware best practices for vMotion.
+
--
...
--
* If you are using vSphere volumes in your pods, migrating a VM across datastores, either manually or through Storage vMotion, causes invalid references within {product-title} persistent volume (PV) objects that can result in data loss.
