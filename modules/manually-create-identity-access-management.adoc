...
You use the CCO to configure Workload Identity Federation (WIF). Cluster Operators use the credential requests that the CCO creates. The installation program does not use these credential requests. The following list outlines the authentication credentials that you require:

* Cluster installation authentication with a service account that uses a {gcp-short} virtual machine (VM)
* Cluster Operators that are configured with the CCO to use WIF.
endif::cco-multi-mode[]
