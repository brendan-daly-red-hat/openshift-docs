...
|The `name` parameter contains the name of the private service connect endpoint.

The optional `region` parameter specifies the location of the endpoint. When the `region` is empty, the location is assumed global.

When you want the installation program to use the public API endpoints and cluster operators to use the API endpoint overrides, set `ClusterUseOnly` to `true`. When you want the installation program and cluster operators to use the API endpoint overrides because you are running the installation program and the cluster in the same network, set `ClusterUseOnly` to `false` . This parameter is optional and defaults to `false`.
