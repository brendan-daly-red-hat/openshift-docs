...
ifdef::aws[]
Capacity Reservations on {aws-full} clusters, including On-Demand Capacity Reservations and Capacity Blocks for ML. Add `capacityReservationId` to the `MachineSet` YAML.
endif::aws[]
...
`<capacity_reservation_group>`:: Specifies the ID of the Capacity Reservation group that you want the machine set to use.
endif::azure[]
...
`<capacity_reservation_group>`:: Specifies the ID of the Capacity Block for ML or On-Demand Capacity Reservation that you want the machine set to use.
`<market_type>`:: Specifies the market type to use. The following values are valid:
