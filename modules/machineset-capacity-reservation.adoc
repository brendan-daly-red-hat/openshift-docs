...
ifdef::azure[]
on-demand Capacity Reservation with Capacity Reservation groups on {azure-full} clusters.
endif::azure[]

ifdef::aws[]
Capacity Reservations on {aws-full} clusters, including On-Demand Capacity Reservations and Capacity Blocks for ML.
endif::aws[]
...
`spec.template.spec.providerSpec.value.capacityReservationGroupID`:: Specifies the ID of the

ifdef::azure[Capacity Reservation group]
`spec.template.spec.providerSpec.value.capacityReservationId`:: Specifies the ID of the
ifdef::aws[Capacity Block for ML or On-Demand Capacity Reservation]
that you want the machine set to deploy machines on.

ifdef::aws[]
`spec.template.spec.machines_v1beta1_machine_openshift_io.spec.providerSpec.value.marketType`:: Specifies the market
