...
[role="_abstract"]
You can define and apply a compute machine set on bare metal to automate node provisioning.
...
ifdef::infra[]
`node-role.kubernetes.io/infra: ""`. It specifies the `infra` node label to add. Edit the `<image_checksum>` URL to use the API VIP address.
endif::infra[]
...
[NOTE]
====
`<image_checksum>`:: Specifies the image checksum in the following format:  `\http://172.22.0.3:6181/images/rhcos-<version>.<architecture>.qcow2.<md5sum>`.

`<image_url>`:: Specifies the image URL in the following format: `\http://172.22.0.3:6181/images/rhcos-<version>.<architecture>.qcow2`.
====
ifeval::["{context}" == "creating-infrastructure-machinesets"]
:!infra:
endif::[]
