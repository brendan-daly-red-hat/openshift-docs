...
`<checksum_url>`:: Edits the `checksum` URL to use the API VIP address.
`<image_url>`:: Edits the `url` URL to use the API VIP address.
endif::infra[]
ifdef::infra[]
`<role>`:: Specifies the node label to add.
`<infra>`:: Specifies the node label to add.
`<infrastructure_id>-infra`:: Specifies the infrastructure ID and node label.
`node-role.kubernetes.io/<role>`:: Specifies the node label to add.
`node-role.kubernetes.io/infra`:: Specifies the node label to add.
`taints`:: Edits the `checksum` URL to use the API VIP address.
endif::infra[]
