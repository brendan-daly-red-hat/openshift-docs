...
`metadata.spec.template.spec.providerSpec`:: The values in the `<providerSpec>` section of the compute machine set CR are platform-specific. For more information about `<providerSpec>` parameters in the CR, see the sample compute machine set CR configuration for your provider.
...
where:
`spec.providerSpec.value.credentialsSecret.name`:: Specifies the name of the secret in the `openshift-machine-api` namespace that contains the required vCenter credentials.
`spec.providerSpec.value.dataDisks`:: Specifies the collection of data disk definitions. For more information, see "Configuring data disks by using machine sets".
`spec.providerSpec.value.template`:: Specifies the name of the {op-system} VM template for your cluster that was created during installation.
`spec.providerSpec.value.network.userDataSecret.name`:: Specifies the name of the secret in the `openshift-machine-api` namespace that contains the required Ignition configuration credentials.
`spec.providerSpec.value.workspace.server:: Specifies the IP address or fully qualified domain name (FQDN) of the vCenter server.
