...
`<hostname>`:: Replaces `<hostname>` with the name of the host.
`<name>`:: Replaces `<name>` with the name of the setting.
`<value>`:: Replaces `<value>` with the value of the setting. You can set multiple name-value pairs.
