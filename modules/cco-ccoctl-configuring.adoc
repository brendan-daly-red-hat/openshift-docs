...
//Below ifeval added to reflect OSDOCS-12321
ifeval::["{context}" == "installing-gcp-customizations"]
[IMPORTANT]
+
====
Find the identity pool ID and the provider ID. During cluster installation, the names of these resources is provided to `ccoctl` with the `--name parameter`.
====
endif::[]
+
[source,terminal]
