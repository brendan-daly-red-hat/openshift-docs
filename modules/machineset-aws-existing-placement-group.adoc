...
[role="_abstract"]
You can configure a machine set to deploy machines on Elastic Fabric Adapter (EFA) instances within an existing {aws-first} placement group. Using Elastic Fabric Adapter (EFA) instances to run control plane machines can improve network performance.
...
`spec.template.spec.providerSpec.value.placement.placementGroupName`:: Specifies the name of the existing {aws-short} placement group to deploy machines in.
`spec.template.spec.providerSpec.value.placement.placementGroupPartition`:: Specifies the partition number of the existing AWS placement group to deploy machines in.
--
