...
`userData.txt`:: Specify `userData.txt` as the name of the new custom secret.
...
`metadata.name`:: Specifies the name of the storage class. This procedure uses `ultra-disk-sc` for this value.
...
`volumeBindingMode`:: Optional: Specifies this parameter to wait for the creation of the pod that will use the disk.
...
`metadata.name`:: Specifies the name of the PVC. This procedure uses `ultra-disk` for this value.
`spec.storageClassName`:: References the `ultra-disk-sc` storage class.
