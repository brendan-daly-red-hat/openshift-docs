...
`<infrastructure_id>-<role>-subnet`:: Specifies the infrastructure ID, node label, and region.
[NOTE]
====
The value of the `<userDataSecret.name>` parameter specifies the node label to add.

The value of the `<value.location>` parameter specifies the region to place machines on.

Use `<value.zone>` to specify the zone within your region to place machines on. Be sure that your region supports the zone that you specify.
====
