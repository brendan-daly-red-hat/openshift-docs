...
==== Cluster API replaces Terraform for {azure-short} Stack Hub

In {product-title} {product-version}, the installation program uses the Cluster API instead of Terraform to provision clusters during installer-provisioned infrastructure (IPI) installations on {azure-short} Stack Hub.
...
[id="ocp-postinstallation-configuration_{context}"]
=== Postinstallation configuration
