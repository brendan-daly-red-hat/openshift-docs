...
[role="_abstract"]
To automate the provisioning and scaling of node virtual machines (VMs) on {rh-openstack-first} for compute workloads, define a `MachineSet` YAML file that specifies details, for example image and network, that are specific to {rh-openstack}.
